(function () {
      ...
      ? courses
        ...
        <div class="library-item">
          ...
          <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
            ${c.course_id ? `<a class="btn btn-gold" href="course.html?id=${encodeURIComponent(c.course_id)}&learn=1">Учиться →</a>` : ""}
            ${c.course_id ? `<a class="btn btn-outline" href="course.html?id=${encodeURIComponent(c.course_id)}">Описание</a>` : c.url ? `<a class="btn btn-outline" href="${escapeHtml(c.url)}" target="_blank" rel="noopener">Партнёр →</a>` : ""}
          </div>
        </div>`
          )
          .join("")
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    await HartI18n.init();
    if (window.HartSession?.isLocalDev?.()) {
