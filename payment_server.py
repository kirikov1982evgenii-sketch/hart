...
load_env_local()
DEV_LOCAL = os.getenv("DEV_LOCAL", "").strip().lower() in ("1", "true", "yes")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/dev/session" and DEV_LOCAL:
            email = os.getenv("MARKET_ADMIN_EMAIL", "eg1982@mail.ru").lower().strip()
            if "@" not in email:
                email = "eg1982@mail.ru"
            ensure_user(email, "Администратор")
            course_id = (q.get("course_id") or [""])[0].strip()
            if course_id:
                ...
                for cid, title in (
                    ("облако-знаний", "Облако знаний"),
                    ("nptel-india", "NPTEL India"),
                    ("microsoft-learn", "Microsoft Learn"),
                    ("hart-lang-python", "Python — полный курс HART"),
                ):
                    add_course_to_library(
                        email,
                        {
            ...
            return self._json(
                200,
                {
                    ...
                    "name": "Администратор",
                    "is_admin": True,
                    "course_granted": course_id or None,
                },
            )
